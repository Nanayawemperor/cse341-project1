...
const getAll = async (req, res) => {
    ...
    const result = await mongodb.getDatabase().db('employees_info').collection('personal_info').find();
...
const getSingle = async (req, res) => {
    ...
    const result = await mongodb.getDatabase().db('employees_info').collection('personal_info').find({ _id: personal_infoId});
    result.toArray().then((personal_info) => {
        res.setHeader('content-Type', 'application/json');
...
const createPersonal_info = async (req, res) => {
    ...
    const response = await mongodb.getDatabase().db('employees_info').collection('personal_info').insertOne(personal_info);
...
const updatePersonal_info = async (req, res) => {
    ...
    const response = await mongodb.getDatabase().db('employees_info').collection('personal_info').replaceOne({ _id: personal_infoId}, personal_info);
...
const deletePersonal_info = async (req, res) => {
    //#swagger.tags = ['personal_info']
    const personal_infoId = new ObjectId(req.params.id);
    const response = await mongodb.getDatabase().db('employees_info').collection('personal_info').deleteOne({ _id: personal_infoId}, true);
